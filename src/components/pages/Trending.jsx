import React, { useEffect, useState } from "react";
import { useNavigate } from "react-router-dom";
import Dropdown from "../templates/Dropdown";
import SearchBar from "../SearchBar";
import axios from "../../Axios";
import GridCards from "../GridCards";
import Loading from "../Loading";
import InfiniteScroll from "react-infinite-scroll-component";
import { MdKeyboardArrowLeft } from "react-icons/md";
import LoadMoreLoader from "../LoadMoreLoader";
import { useSearchParams } from "react-router-dom";

const Trending = () => {
  const navigate = useNavigate();

  const [searchParams, setSearchParams] = useSearchParams();
  const category = searchParams.get("category") || "all";
  const duration = searchParams.get("duration") || "week";
  const [trendings, setTrendings] = useState([]);
  const [loading, setLoading] = useState(true);
  const [hasMore, setHasMore] = useState(true);
  const [page, setPage] = useState(1);

  const fetchTrendings = async (currentPage = page) => {
    try {
      setLoading(true);
      const { data } = await axios.get(
        `/trending/${category}/${duration}?page=${currentPage}`,
      );

      if (data.page >= data.total_pages) {
        setHasMore(false);
      }

      setTrendings((prev) => {
        const merged = [...prev, ...data.results];
        return merged.filter((item, index, self) => {
          return (
            index ===
            self.findIndex((current) => {
              return (
                current.id === item.id && current.media_type === item.media_type
              );
            })
          );
        });
      });
    } catch (error) {
      console.log(error);
    } finally {
      setLoading(false);
    }
  };

  const loadMore = () => {
    setPage((prev) => prev + 1);
  };

  useEffect(() => {
    setTrendings([]);
    setHasMore(true);
    setPage(1);

    fetchTrendings(1);
  }, [category, duration]);

  useEffect(() => {
    if (page > 1) {
      fetchTrendings(page);
    }
  }, [page]);

  if (loading && trendings.length === 0) {
    return <Loading />;
  }

  const title =
    category === "all"
      ? "Trending"
      : `Trending ${category === "movie" ? "Movies" : "TV Shows"}`;

  const subtitle =
    category === "all"
      ? duration === "week"
        ? "Explore what's trending worldwide this week."
        : "See what's trending around the world today."
      : category === "movie"
        ? duration === "week"
          ? "Explore the most talked-about movies this week."
          : "See today's trending movies."
        : duration === "week"
          ? "Explore the most talked-about TV shows this week."
          : "See today's trending TV shows.";

  return (
    <div className="w-full px-4 md:px-10 py-7">
      <button
        onClick={() => navigate("/")}
        className="group inline-flex items-center gap-0.5 text-zinc-400 hover:text-white transition-colors duration-300 mb-3 cursor-pointer"
      >
        <span className="transition-transform duration-300 group-hover:-translate-x-1">
          <MdKeyboardArrowLeft size={20} />
        </span>
        <span className="text-base font-medium">Back</span>
      </button>
      <div className="sticky top-0 z-40 py-4 bg-[#222222]/90 backdrop-blur-md">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          <div className="w-full lg:w-auto lg:max-w-md">
            <h1 className="text-xl md:text-3xl font-bold text-white">{title}</h1>

            <p className="text-zinc-400 mt-1 md:mt-2 text-sm md:text-base">{subtitle}</p>
          </div>

          <div className="w-full lg:flex-1 lg:ml-8 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
            <SearchBar mediaType={category === "all" ? "all" : category} />
            <div className="flex items-center gap-2 sm:gap-3">
              <Dropdown
                value={category}
                options={[
                  { label: "All", value: "all" },
                  { label: "Movie", value: "movie" },
                  { label: "TV Show", value: "tv" },
                ]}
                category={(value) =>
                  setSearchParams({
                    category: value,
                    duration,
                  })
                }
              />
              <Dropdown
                value={duration}
                options={[
                  { label: "Week", value: "week" },
                  { label: "Day", value: "day" },
                ]}
                category={(value) =>
                  setSearchParams({
                    category,
                    duration: value,
                  })
                }
              />
            </div>
          </div>
        </div>
      </div>
      <div className="mt-6">
        <InfiniteScroll
          dataLength={trendings.length}
          next={loadMore}
          hasMore={hasMore}
          loader={<LoadMoreLoader />}
          endMessage={
            <div className="py-10 text-center">
              <p className="text-zinc-400">🍿 That's everything we've got.</p>

              <p className="text-sm text-zinc-500 mt-1">
                Try another category or switch between Day and Week.
              </p>
            </div>
          }
        >
          <GridCards data={trendings} />
        </InfiniteScroll>
      </div>
    </div>
  );
};

export default Trending;
