...
const Popular = () => {
  ...
  const fetchPopular = async (currentPage = page) => {
    try {
      setLoading(true);
      const { data } = await axios.get(
        `${category}/popular?page=${currentPage}`,
      );
          ...
          endMessage={
            <div className="py-10 text-center">
              ...
              <p className="text-sm text-zinc-500 mt-1">
                Explore the other category for more titles.
              </p>
            </div>
          }
        >
          <GridCards mediaType={category} data={popular} />
        </InfiniteScroll>
      </div>
    </div>
  );
